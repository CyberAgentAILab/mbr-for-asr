...
"""
The following code is added by Yuu Jinnai.
"""
class METRICX(UtilityFunction):
    """MetricX class."""
    ...
    def __init__(self, model_id: str):
        ...
        self.model = MT5ForRegression.from_pretrained(model_id, device_map="cuda")

        self.prompt = "candidate: " + "{hyp}" + " reference: " + "{ref}"

    def compute_similarity(self, hyp, ref, src=None):
        hyp = list(hyp)
        ref = list(ref)

        n_comparisons = len(hyp)
        max_length = max([len(h) for h in hyp])
        token_size = n_comparisons * max_length // 2  # rough estimate of token size
        ...
        if token_size > 4096:
            ...
            for i in range(len(hyp)):
                ...
                inputs = self.tokenizer(
                    self.prompt.format(hyp=hi, ref=ri),
                    return_tensors="pt",
                    padding=False,
                ...
                with torch.no_grad():
                    output = self.model(**inputs).predictions.cpu().detach().item()
                    score = 1.0 - output / 25.0
                scores.append(score)
        else:
            texts = []
            for i in range(len(hyp)):
                hi = hyp[i]
                ri = ref[i]
                texts.append(self.prompt.format(hyp=hi, ref=ri))
            inputs = self.tokenizer(
                texts,
                return_tensors="pt",
                padding=True,
            ...
            with torch.no_grad():
                output = self.model(**inputs).predictions.cpu().detach().numpy()
            scores = np.ones_like(output) - output / 25.0
